...
""".replace('{DataTable}', str('DataTable' if not os.environ.get('TABLENAME') else os.environ.get('TABLENAME')))
)
...
class CellRow(
    ...
    def __init__(self, **kwargs):
        ...
        self.ids.check.parent.remove_widget(self.ids.check)
...
class TableHeader(ScrollView):
    ...
    def on_table_data(self, instance, value):
        ...
        if self.table_data.check:
            self.ids.check.size = (dp(32), dp(32))
            self.ids.check.opacity = 1 if self.ids.check.opacity != 0 else self.ids.check.opacity
        else:
            self.ids.box.padding[0] = 0
            self.ids.box.spacing = 0
...
class DataTable(ThemableBehavior, AnchorLayout):
